...
-- 1. Yearly Revenue --
CREATE OR REPLACE VIEW finalBK.TS_yearly_revenue AS
SELECT
    d.year_num,
...
-- 2. Monthly Revenue --
CREATE OR REPLACE VIEW finalBK.TS_monthly_revenue AS
SELECT
    d.year_num,
    d.month_num,
...
CREATE OR REPLACE VIEW finalBK.TS_yearly_revenue_by_store AS
SELECT
    d.year_num,
    st.store_name,
    ROUND(SUM(s.sale_price * s.quantity), 2) AS Yearly_Rev
...
CREATE OR REPLACE VIEW finalBK.TS_yearly_category_performance AS
SELECT 
    c.category_name,
    d.year_num,
...
CREATE OR REPLACE VIEW finalBK.TS_yoy_revenue_change AS
...
CREATE OR REPLACE VIEW finalBK.TS_mom_revenue_change AS
...
CREATE OR REPLACE VIEW finalBK.TS_brand_yoy_change AS
